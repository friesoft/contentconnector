...
public abstract class IndexUpdateChecker {
	...
	/**
	 * Checks the identifier/timestamp pair against the index iterator if the object in the index exists and if it is up to date.
	 ...
	 */
	public boolean isUpToDate(final String identifyer, final Object timestamp, final String timestampattribute, final Resolvable object)
			throws WrongOrderException {
		if (!"".equals(this.lastIdentifier) && this.lastIdentifier.compareTo(identifyer) < 0) {
	...
	/**
	 * delete the objects that are not more existent. 
	 * usually this is done by holding a list of all objects created at initialization and removing all checked objects from the list. 
	 * this leaves all not longer existent objects in the list.
	 */
